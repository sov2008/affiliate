...
async function main() {
  ...
  try {
    ...
    console.log(`\n${colors.bold}✍️  STAGE 1: GENERATED COPY & ANGLE:${colors.reset}`);
    console.log(`  ${colors.yellow}${colors.bold}Hook:${colors.reset} "${result.copy.hook}"`);
    console.log(`  ${colors.dim}Body:${colors.reset} ${result.copy.body}`);
    console.log(`  ${colors.green}${colors.bold}CTA:${colors.reset}  ${result.copy.callToAction}`);
    console.log(`  ${colors.dim}Angle Strategy:${colors.reset} ${result.copy.angle}`);

    console.log(`\n${colors.bold}🛡️  STAGE 2: COMPLIANCE GATEKEEPER:${colors.reset}`);
    ...
    console.log(`\n${colors.bold}🎨 STAGE 3: VISUAL PROMPT CRAFTING:${colors.reset}`);
    ...
    console.log(`\n${colors.bold}🖼️  STAGE 4: GENERATED ASSET & STORAGE URL:${colors.reset}`);
    ...
    console.log(`  - Angle Generation:     ${result.telemetry.stages.angleGenMs}ms`);
    console.log(`  - Compliance Check:     ${result.telemetry.stages.complianceCheckMs}ms`);
    console.log(`  - Visual Prompt Craft:  ${result.telemetry.stages.promptCraftMs}ms`);
    console.log(`  - Image Gen & Upload:   ${result.telemetry.stages.imageGenAndUploadMs}ms`);
