...
**Дата и время генерации:** 2026-09-01T09:00:01.821Z  
...
| :--- | :--- | :--- | :---: | :---: |
| **Case A: Высокоинтентная органическая тема** | `COMPLIANT` / Score ≥ 80 | `COMPLIANT` (Score: **92/100**) | **1970 ms** | ✅ **ПРОЙДЕН** |
| **Case B: Агрессивный спам и стоп-слова** | `REJECTED` / Score < 80 | `REJECTED` (Score: **0/100**, Триггеров: **9**) | **1 ms** | ✅ **ПРОЙДЕН** |
| **Case C: Аварийный E-STOP в процессе** | `HALTED` (Мгновенный обрыв) | `HALTED` (E-STOP Lock: **АКТИВЕН**) | **2 ms** | ✅ **ПРОЙДЕН** |
...
- **Время реакции E-STOP (Circuit Breaker Trigger-to-Halt)**: **2 ms** (Атомарный локфайл `EMERGENCY_STOP.lock`).
...
- **Всего бандлов сохранено на диске**: **12**.
